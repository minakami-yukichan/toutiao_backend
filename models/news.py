...
class News(Base):
    ...
    __table_args__ = (
        Index("fk_news_category_idx", "category_id"),  # 高频查询场景，为 category_id 创建索引 
        Index("idx_publish_time", "publish_time")  # 按发布时间条件
    )
    ...
    def __repr__(self) -> str:
        return f"News(id={self.id}, title={self.title}, category_id={self.category_id}), image={self.image}"